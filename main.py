...
def goMulti(parent=None):
    ...
    mainframe.pack()
    lab = tkinter.Label(mainframe,text="Choose whether you're a\n host or a client:\n")
    lab['font'] = font.Font(size=20)
    lab.pack()
    btn1 = tkinter.Button(mainframe,text="Play as host",width=18,height=5,command=lambda : goHost(wnd))
    btn1['font'] = font.Font(size=14)
    btn1.pack()
    btn2 = tkinter.Button(mainframe,text="Play as client",width=18,height=5,command=lambda : goClient(wnd))
...
def goHome(parent=None):
    ...
    lab['font'] = font.Font(size=20)
    lab.pack()
    btn1 = tkinter.Button(mainframe,text="Multiplayer",height=5,width=15,command=lambda : goMulti(wnd))
    btn1['font'] = font.Font(size=14)
    btn1.pack()
    btn2 = tkinter.Button(mainframe,text="Single Player",height=5,width=15,command=lambda : goSolo(wnd))
    btn2['font'] = font.Font(size=14)
    btn2.pack()
    wnd.mainloop()
